...
[0.9034502399999998, 0.9410939999999999, 0.97, 0.35031744, 0.0090345024, 1.0, 8.582777280000001e-05]
"""
...
def are_names(s,delete_last_empty=True):
  ...
  for x in s:
    ...
    if len(xs)<=0:
      ...
      if not xs.istitle():
        c0 *= name_confidence_factors['is_title'];
      if not xs.isalpha():
        c0 *= .97
      # length criteria
...
def cleanpunc(s=''):
  """Delete any spaces before periods or question marks at the end of a sentence. Reduce double spaces between sentences to single spaces. Not implemented
